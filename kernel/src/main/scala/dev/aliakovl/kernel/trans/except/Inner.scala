...
private object Inner:
  ...
  given [M[+_]: Monad, E]: Monad[[A] =>> ExceptT[M, E, A]] with
    def pure[A](a: A): ExceptT[M, E, A] = ExceptT(
      summon[Monad[M]].pure(Right(a))
    )
  ...
  extension [M[+_], E, A](ta: ExceptT[M, E, A])
    def mapExceptT[N[+_], E1, B](
        f: M[Either[E, A]] => N[Either[E1, B]]
    ): ExceptT[N, E1, B] = ExceptT(f(ta.runExceptT))
  ...
  extension [M[+_]: Monad, E, A](ta: ExceptT[M, E, A])
    ...
    def fold[E1, B](
        failure: E => ExceptT[M, E1, B],
        success: A => ExceptT[M, E1, B]
    ): ExceptT[M, E1, B] = ExceptT {
  ...
  def handleE[M[+_]: Monad, E, E1, A](f: E => ExceptT[M, E1, A])(
      t: ExceptT[M, E, A]
  ): ExceptT[M, E1, A] =
    t.catchE(f)
