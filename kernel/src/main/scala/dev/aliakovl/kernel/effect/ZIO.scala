...
object ZIO:
  ...
  def suspendSucceed[R, E, A](zio: => ZIO[R, E, A]): ZIO[R, E, A] = ZIO.success(zio).flatMap(x => x)
  ...
  extension[R, E, A] (zio: ZIO[R, E, A])
    ...
    def catchAll[R1 <: R, E1, A1 >: A](h: E => ZIO[R1, E1, A1]): ZIO[R1, E1, A1] = foldZIO(h, success)
    ...
    def refineOrDie[E1](pf: PartialFunction[E, E1])(implicit ev: E <:< Throwable): ZIO[R, E1, A] = {
    ...
    def exit: ZIO[R, Nothing, Either[E, A]] = fromReader[R, Nothing, Either[E, A]] { r =>
      zio.runReaderT(r).tryT
    }

  extension[R, E <: Throwable, A] (zio: ZIO[R, E, A])
    def refineToOrDie[E1 <: E]: ZIO[R, E1, A] = zio.refineOrDie { case e: E1 => e }
