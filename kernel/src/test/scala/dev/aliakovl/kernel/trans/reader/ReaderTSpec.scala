...
extension [A](either: Either[Nothing, A]) def getSafe: A = either.toOption.get
...
object ReaderTSpec:
  ...
  def rightString(min: Int, max: Int): ZIO[String, LengthException, Int] = for {
    ...
    _ <-
      if n > max then ReaderT.lift[M, String, Unit](throwE(ToLong))
      else if n < min then ReaderT.lift[M, String, Unit](throwE(ToShort))
      else ReaderT.pure[M, String, Unit](())
  } yield n

  val program: ZIO[String, Nothing, String] =
    ReaderT[[T] =>> Except[Nothing, T], String, String] { r =>
      rightString(1, 6)
        .map(_.toString)
        .runReaderT(r)
        .catchE(e => ExceptT.pure(e.toString))
    }
