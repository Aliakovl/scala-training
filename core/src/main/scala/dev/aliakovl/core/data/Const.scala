...
object Const {
  ...
  implicit def constApplicative[C: Monoid]
      : Applicative[({ type λ[α] = Const[C, α] })#λ] =
    new Applicative[({ type λ[α] = Const[C, α] })#λ] {
      override def pure[A](a: A): Const[C, A] = Const(
        implicitly[Monoid[C]].empty
      )

      override def ap[A, B](ff: Const[C, A => B])(
          fa: Const[C, A]
      ): Const[C, B] =
        Const(fa.value)
    }
